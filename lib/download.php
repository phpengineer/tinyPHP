...
class Download {
	 ...
	 * @param type $isdebug
	 */
	public function init($file_filter = '', $isdebug = true) {
		$this->setFilter($file_filter);
		$this->setDebug($isdebug);
		$this->setFileType();
	}
	
	protected function setFilter($fileFilter) {
		if(empty($fileFilter)) {
			return;
		}
	...
	protected function fileCheck() {
		...
		if(file_exists($filename)) {
			...
			if(in_array($filetype, $this->filter)) {
				$this->errormsg .= filename . ' could not be downloaded';
				if($this->debug) {
					exit($filename . ' could not be downloaded');
				}
				return false;
			} else {
				...
				if(empty($this->mimeType)) {
					if(isset($this->xlq_filetype[$filetype])) {
						$this->mimeType = $this->xlq_filetype[$filetype];
					}
				}
				if(!empty($this->mimeType)) {
					return true;
